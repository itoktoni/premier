	...
	<table id="export" border="1" style="border-collapse: collapse !important; border-spacing: 0 !important;"
		...
		<tbody>
			...
			<tr>
				...
				<td>
					@if ($table->opname_detail_ketemu == 1)
					{{ formatDate($table->opname_detail_waktu) }}
					@else
					{{ formatDate($table->opname_detail_updated_at) }}
					@endif
				</td>
				<td>
					@if ($table->opname_detail_ketemu == 1)
					{{ \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', $table->opname_detail_waktu)->diff(now())->format('%a') }}
					@else
					{{ $table->opname_detail_updated_at ? \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', $table->opname_detail_updated_at)->diff(now())->format('%a') : '0' }} Hari
					@endif
				</td>
