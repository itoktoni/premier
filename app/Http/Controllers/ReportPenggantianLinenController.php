...
class ReportPenggantianLinenController extends MinimalController
{
    public $data;
    ...
    {
        $query = self::$repository->getConfig()
            ->join(Detail::getTableName(), 'detail_rfid', 'view_linen_rfid')
            ->orderBy('view_linen_nama', 'ASC');

            if($request['filter_start'])
            {
                $query = $query->whereDate('detail_pengantian_waktu', '>=', $request['filter_start']);
            }

            if($request['filter_end'])
            {
                $query = $query->whereDate('detail_pengantian_waktu', '<=', $request['filter_end']);
            }
